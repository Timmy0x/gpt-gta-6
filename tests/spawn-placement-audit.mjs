...
const origin=process.env.AUDIT_URL||'http://127.0.0.1:4182',backend=process.env.AUDIT_BACKEND||'webgpu',out=`docs/evidence/safe-spawn-${backend}`;await fs.mkdir(out,{recursive:true});
...
try{
 ...
 for(let i=0;i<2;i++){
  await page.locator('[data-action="spawn"]').click();await page.waitForFunction(expected=>window.__leonida.game.vehicles.list.filter(v=>v.kind==='concept').length===expected,i+1,{timeout:45000});await page.waitForTimeout(1500);const s=await state();checks.push({name:`concept-${i+1}`,state:s});
