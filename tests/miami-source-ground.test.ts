...
test('compiled common-frame Brickell payload supports native negative ground, roads and source building faces without invented waterbeds or bridge decks',async t=>{
 ...
 assert.equal(manifest.worldId,'brickell-public-common-frame-r2');
 ...
 const buildingBodies=new Map<string,PhysicsAggregate>();
 ...
 for(const chunk of manifest.chunks){const bytes=await readFile(new URL(chunk.url,root));assert.equal(createHash('sha256').update(bytes).digest('hex'),chunk.sha256);const decoded=decodeMiamiChunk(chunk,bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.byteLength));
  for(const d of decoded){assert.notEqual(d.record.kind,'waterbed');assert.notEqual(d.record.material,'waterbed');assert.equal(d.record.collision,true);const mesh=new Mesh(d.record.id,scene),data=new VertexData();data.positions=d.positions;data.normals=d.normals;data.indices=d.indices;data.uvs=d.uvs;data.applyToMesh(mesh);mesh.position=Vector3.FromArray(d.record.origin);mesh.computeWorldMatrix(true);const body=new PhysicsAggregate(mesh,PhysicsShapeType.MESH,{mass:0,friction:d.record.friction,restitution:0},scene);body.shape.filterMembershipMask=d.record.kind==='building'?4:d.record.kind==='road'?1:d.record.kind==='terrain'?2:8;meshes++;triangles+=d.indices.length/3;if(d.record.kind==='building'){buildingMeshes++;buildingBodies.set(d.record.id,body);body.shape.filterMembershipMask=4;for(const wall of [false,true])for(let i=0;i<d.indices.length;i+=3){const ids=Array.from(d.indices.slice(i,i+3)),normal=ids.reduce((n,id)=>n.add(Vector3.FromArray(d.normals,id*3)),Vector3.Zero()).normalize();if(wall?Math.abs(normal.y)>.2:normal.y<.9)continue;const point=ids.reduce((p,id)=>p.add(Vector3.FromArray(d.positions,id*3)),Vector3.Zero()).scale(1/3).add(mesh.position);buildingFaces.push({point,normal,id:d.record.id,mask:4,wall});}}}
 }
 ...
 for(const [key,faces] of faceGroups){const building=buildingBodies.get(faces[0].id)!;building.shape.filterMembershipMask=16;let exposed=false;faces.sort((a,b)=>a.wall?b.point.x-a.point.x:b.point.y-a.point.y);for(const {point,normal,mask} of faces){const hit=physics.raycast(point.add(normal.scale(.02)),point.subtract(normal.scale(.02)),{collideWith:16});if(hit.hasHit&&hit.body?.transformNode.name===key.slice(0,key.lastIndexOf(':'))&&Vector3.Distance(hit.hitPointWorld,point)<.001){exposed=true;buildingFaceChecks++;break;}nonExposedCandidates++;}building.shape.filterMembershipMask=4;assert.ok(exposed,'no verified exposed source face in '+key);}

 const ray=(x:number,z:number,expected:number,mask:number)=>{const hit=physics.raycast(new Vector3(x,expected+3,z),new Vector3(x,expected-3,z),{collideWith:mask});assert.ok(hit.hasHit,`missing real support at ${x},${z}`);assert.ok(hit.hitNormalWorld.y>0,'terrain support can be too steep for walking; travel separately requires a walkable normal');return hit.hitPointWorld.y;};
 ...
 assert.equal(buildingMeshes,262);assert.ok(groundChecks>200);assert.ok(roadChecks>30);assert.ok(maxGroundError<.06,'converted source DEM tessellation approximation');assert.ok(maxRoadError<.06,'converted DEM road surface tessellation');assert.ok(restingError<.08);
});
