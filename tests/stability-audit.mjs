...
const output=path.resolve('docs/evidence/stability',`${process.argv.includes('--fingerprint-only')?'fingerprint':flightOnly?'flight-check':smoke?'smoke':'baseline'}-${stamp}`);
...
async function destructionUntil(end){await keys();await page.evaluate(async()=>{const g=window.__leonida.game,Vec=g.player.position.constructor;await g.world.preparePosition(new Vec(-288,1.2,140));g.player.exit(true);g.player.teleport(new Vec(-288,1.2,140));g.player.yaw=0;g.player.pitch=-.1;});await event('hook-fixture-foot-position',{position:[-288,1.2,140]});await creative({wanted:4,god:true,ammo:true});let rounds=0,next=0;
...
async function mixedUntil(end){const ids=['palma-market','mangrove-estates','restricted-compound','marina','ocean-drive','south-wharf'];let visit=0;
  while(performance.now()<end){const id=ids[visit%ids.length];await panel('map');await page.locator(`[data-action="teleport"][data-value="${id}"]`).click();await page.waitForFunction(()=>!window.__leonida.snapshot().streamingBusy);await event('creative-ui-fast-travel',{location:id});await tick(1000);await page.keyboard.press('Tab');await keys(['w']);await tick(smoke?1000:5000);await keys();await panel('creative');await page.locator('[data-action="save"]').click();await tick(300);await page.locator('[data-action="load"]').click();await page.waitForFunction(()=>!window.__leonida.snapshot().streamingBusy);await closePanel();await event('creative-ui-save-load',{visit,location:id});visit++;const segmentEnd=Math.min(end,performance.now()+(smoke?1500:25000));while(performance.now()<segmentEnd){await keys(['w']);await tick(500);}await keys();}
}
