...
import {decodeMiamiChunk,validateMiamiManifest} from '../vendor/world/miami/MiamiPackages';
import {createPublicCollisionQueries} from '../src/PublicCollisionQueries';
const root=new URL('../output/',import.meta.url),json=async(n:string)=>JSON.parse(await readFile(new URL(n,root),'utf8'));
test('actual common-frame Brickell packages support native ground, road and resting body without waterbed',async t=>{
 ...
 for(let x=dataset.bounds.minX+8;x<dataset.bounds.maxX-8;x+=19)for(let z=dataset.bounds.minZ+8;z<dataset.bounds.maxZ-8;z+=19){const q=queries.surfaceAt(x,z);if(!q)continue;const actual=ray(x,z,q.point[1],2);maxGroundError=Math.max(maxGroundError,Math.abs(actual-q.point[1]));groundChecks++;}
 ...
 const junction=dataset.locations[0],q=queries.surfaceAt(junction.point[0],junction.point[2])!;assert.ok(q);const roadY=ray(q.point[0],q.point[2],q.point[1]+.025,1);
 ...
 await writeFile(new URL('native-result.json',root),JSON.stringify(report,null,2)+'\n');t.diagnostic(JSON.stringify(report));
 assert.equal(buildingMeshes,20);assert.ok(groundChecks>200);assert.ok(roadChecks>30);assert.ok(maxGroundError<.12,'source DEM tessellation approximation');assert.ok(maxRoadError<.12,'DEM surface tessellation');assert.ok(restingError<.08);
});
