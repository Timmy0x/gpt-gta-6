...
function focus(radius=420){camera.center=new Vector3(...ecef(25.7662,-80.1907,0));camera.radius=radius;camera.pitch=1.1;camera.yaw=-.25;}
focus();
interface Session {config:Configuration;tiles:TilesRenderer;loaded:number;root:boolean;generation:number;failure:boolean;diagnostics:RequestDiagnostics;endpointCredits:Credit[];}
...
function connect(config:Configuration){disconnect();saved={...config};const tiles=new TilesRenderer(rootUrl(config),scene);const session:Session={config:saved,tiles,loaded:0,root:false,generation:++generation,failure:false,diagnostics:new RequestDiagnostics(),endpointCredits:[]};current=session;
 ...
 tiles.addEventListener('load-model',()=>{if(current===session)session.loaded++;});
 ...
 state.textContent='Connecting…';credential.value='';focus();
}
