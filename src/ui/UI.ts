...
export class UI {
  ...
  constructor() {
    ...
    <main id="welcome" class="hidden"><div class="welcome-left"><div class="edition"><i></i> LOCAL DEVELOPMENT BUILD <span>01</span></div><p class="eyebrow">VICE CITY, LEONIDA</p><h1>MAKE YOUR<br>OWN <em>TROUBLE.</em></h1><p class="intro">The coast is yours. Take a walk, find a ride,<br>and see where the evening takes you.</p><button class="primary" data-action="play">ENTER FREE ROAM <span>↗</span></button><button class="text-button" data-action="continue">Continue saved sandbox <span>→</span></button><p id="welcome-status" role="status" aria-live="polite"></p><div class="welcome-controls"><span><kbd>W A S D</kbd> Move</span><span><kbd>E</kbd> Enter a vehicle</span><span><kbd>F2</kbd> Create anything</span></div></div><div class="welcome-foot"><span>LEONIDA / FREE ROAM</span><span>Original and licensed assets · Reconstruction in progress</span><span>BABYLON.JS × HAVOK</span></div></main>
    <div id="hud" class="hidden"><header class="hud-header"><div class="brand">LEONIDA <span>FREE ROAM</span></div><div class="top-right"><span id="clock">18:24</span><span class="weather" id="weather">CLEAR</span><button data-action="map" title="World map (M)">MAP <kbd>M</kbd></button><button data-action="creative" title="Creative mode (F2)">SANDBOX <kbd>F2</kbd></button><button data-action="pause" title="Pause (Escape)">Ⅱ</button></div></header><section class="wanted-area"><div id="stars"></div><div id="wanted-state"></div></section><section class="location-card"><span class="eyebrow">VICE CITY</span><h2 id="district">Ocean Beach</h2><div id="activity">Your city. Your rules.</div></section><div id="crosshair">·</div><div id="toast" class="hidden"></div><div id="prompt"></div><section class="bottom-left"><div class="map-wrap"><canvas id="minimap" width="224" height="184"></canvas><div class="north">N ↑</div><div class="map-caption">OCEAN BEACH <span>↗</span></div></div><div class="vitals"><span id="character">JASON</span><div class="bar"><i id="health-bar"></i></div><div class="bar armor"><i id="armor-bar"></i></div></div></section><section class="bottom-right"><div id="vehicle-hud"></div><div class="weapon-info"><span id="weapon"></span><b id="ammo"></b></div><div id="money"></div><div class="keys-hint"><kbd>E</kbd> Interact <kbd>TAB</kbd> Switch <kbd>ESC</kbd> Pause</div></section><div id="performance"></div></div>
    <aside id="panel" class="hidden"></aside><div id="outcome" class="hidden"></div>`;
