...
export class UI {
  ...
  private bindMap(): void {
    ...
    const select = (point: MapPoint & { name?: string }) => {
      if (!playableMapPoint(point,this.world?.bounds)) { this.mapSelection=null; document.querySelector('#map-point')!.textContent = 'Area not built yet'; for(const button of document.querySelectorAll<HTMLButtonElement>('[data-map-command="route"], [data-map-command="travel"]'))button.disabled=true;redraw();return; }
  ...
  private drawMap(canvas: HTMLCanvasElement, s: HudState, scale: number) {
    ...
    if (geography) {
      ...
      ctx.fillStyle='#34453f';ctx.fillRect(b.minX,b.minZ,b.maxX-b.minX,b.maxZ-b.minZ);
