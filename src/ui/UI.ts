import { ROAD_CARS, ROAD_CAR_KINDS } from '../vehicles/RoadCarCatalog';
const VEHICLE_CHOICES = ROAD_CAR_KINDS.map(kind=>({kind,label:ROAD_CARS[kind].tuning.label}));
...
export class UI {
  ...
  private bindMap(): void {
    ...
    const municipal = this.world?.mapData?.municipalBoundary?.flat(2);
    if (municipal?.length) this.mapView.bounds = { minX: Math.min(...municipal.map(p=>p[0])), maxX: Math.max(...municipal.map(p=>p[0])), minZ: Math.min(...municipal.map(p=>p[1])), maxZ: Math.max(...municipal.map(p=>p[1])) };
    else if (this.world?.bounds) this.mapView.bounds = this.world.bounds;
    ...
    const select = (point: MapPoint & { name?: string }) => {
      if (!playableMapPoint(point,this.world?.bounds) || this.world?.hasGroundCoverage?.(point.x,point.z) === false) { this.mapSelection=null; document.querySelector('#map-point')!.textContent = 'Area not built yet'; for(const button of document.querySelectorAll<HTMLButtonElement>('[data-map-command="route"], [data-map-command="travel"]'))button.disabled=true;redraw();return; }
      ...
      document.querySelector('#map-point')!.textContent = `${point.name ?? 'Map pin'} · ${distance} m`;
  ...
  update(s: HudState) {
    ...
    text(
      "district",
      this.world?.mapData ? "Brickell" : s.position.x > 155
        ? "Ocean Beach"
        : s.position.x < -450
          ? "West Vice City"
          : s.position.x < -95
  ...
  private drawMap(canvas: HTMLCanvasElement, s: HudState, scale: number) {
    ...
    ctx.scale(zoom, -zoom);
    const geography = this.world?.mapData;
    if (geography) {
      const polygon = (rings: number[][][]) => { ctx.beginPath(); for (const ring of rings) { ring.forEach((p,i) => i ? ctx.lineTo(p[0],p[1]) : ctx.moveTo(p[0],p[1])); ctx.closePath(); } };
      ctx.fillStyle='#1c2b2a';ctx.strokeStyle='#485752';ctx.lineWidth=1/zoom;
      for(const area of geography.municipalBoundary??[]){polygon(area);ctx.fill('evenodd');ctx.stroke();}
      const b=geography.bounds;
      ctx.save();ctx.beginPath();ctx.rect(b.minX,b.minZ,b.maxX-b.minX,b.maxZ-b.minZ);ctx.clip();
      ctx.fillStyle='#34453f';for(const area of geography.land)for(const shape of area.polygons){polygon(shape);ctx.fill('evenodd');}
      ctx.fillStyle='#397477';for(const area of geography.water)for(const shape of area.polygons){polygon(shape);ctx.fill('evenodd');}
      ctx.fillStyle='#52615b';for(const building of geography.buildings){polygon(building.footprint);ctx.fill('evenodd');}
      ctx.lineCap='round';ctx.lineJoin='round';
      for(const road of geography.roads){ctx.beginPath();road.centerline.forEach((p,i)=>i?ctx.lineTo(p[0],p[2]):ctx.moveTo(p[0],p[2]));ctx.strokeStyle='#89918b';ctx.lineWidth=road.widthM+2*road.sidewalkWidthM;ctx.stroke();ctx.strokeStyle='#42514d';ctx.lineWidth=road.widthM;ctx.stroke();}
      ctx.restore();ctx.strokeStyle='#c5a977';ctx.lineWidth=1/zoom;ctx.setLineDash([6/zoom,5/zoom]);ctx.strokeRect(b.minX,b.minZ,b.maxX-b.minX,b.maxZ-b.minZ);ctx.setLineDash([]);
    } else {
