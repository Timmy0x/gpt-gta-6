...
const root=resolve(import.meta.dirname,'../../..'),out=resolve(root,'public/characters/civilians');await mkdir(out,{recursive:true});
const records=[];
for(const {name,label,description,female} of variants){
 const bytes=await readFile(resolve(root,'data/characters/civilians/source',name,name+'.fbx'));
 ...
 // FBX joint arrays need not start with their common root. Three exports the
 // first joint as skin.skeleton; Female_Adult_06 starts at Spine2, so select the
 // actual lowest common ancestor without changing joint or inverse-bind order.
 const parents=new Map();json.nodes.forEach((node,index)=>node.children?.forEach(child=>parents.set(child,index)));
 for(const skin of json.skins){const chains=skin.joints.map(joint=>{const chain=[];for(let current=joint;current!==undefined;current=parents.get(current))chain.push(current);return chain;});const common=chains[0].find(node=>chains.every(chain=>chain.includes(node)));if(common===undefined)throw new Error(`No common skeleton root for ${name}`);skin.skeleton=common;}
 ...
 json.asset.extras={source:'Microsoft Rocketbox',character:name,commit,license:'MIT',copyright:'Copyright (c) 2020 Microsoft',changes:'Converted FBX to metre-scaled glTF; selected common skin root; grouped and welded vertices; converted diffuse/normal/alpha textures; PBR roughness authored.'};
 ...
 const meshes=[];fbx.traverse(m=>{if(m.isMesh)meshes.push({vertices:m.geometry.attributes.position.count,triangles:m.geometry.index.count/3,groups:m.geometry.groups.length,bones:m.skeleton?.bones.length});});records.push({name,label,description,female,file:`${label}.glb`,bytes:glb.length,sha256:createHash('sha256').update(glb).digest('hex'),meshes,bounds:new Box3().setFromObject(wrapper)});
}
await writeFile(resolve(out,'manifest.json'),JSON.stringify({format:'glTF2 GLB with external textures',commit,license:'MIT',copyright:'Copyright (c) 2020 Microsoft',assets:records},null,2));console.log(JSON.stringify(records,null,2));
